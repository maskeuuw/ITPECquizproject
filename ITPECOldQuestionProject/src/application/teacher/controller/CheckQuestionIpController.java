...
import application.teacher.form.QuestionLookIpForm;
...
public class CheckQuestionIpController implements Initializable {
...
public void initialize(URL location, ResourceBundle resources) {
    ...
    questionNumberColumn.setCellValueFactory(new PropertyValueFactory<>("qNo"));
    ...
    // Set the items after all data is loaded
    ...
    try {
        ...
        showAlert(AlertType.ERROR, "Database Error", "Data Load Failed",
                "There was an issue retrieving checkbox data from the database.");
    }
}
	...
	private String getChapterName(int chapterId) {
		...
		try (Connection con = new DatabaseConnection().getConnetion();
			...
			e.printStackTrace();
			System.err.println("Error getting chapter name for ID: " + chapterId);
		}
		return "Unknown Chapter (ID: " + chapterId + ")";
	}
	...
	private void loadTableData() throws SQLException {
		...
		String sql = "SELECT * FROM ipquestion WHERE esection_id = 1 ORDER BY q_no ASC";
		PreparedStatement pst = con.prepareStatement(sql);
		ResultSet rs = pst.executeQuery();
	...
	void forQuesCheck(ActionEvent event) throws IOException, SQLException {
		...
		PreparedStatement pst = con.prepareStatement(
				"SELECT * FROM ipquestion WHERE q_no=? AND c_id = ? AND ym_id = ? AND esection_id = 1");
		...
		System.out.println("Question NO Tet :" + qNoText + " selectedChapterCheckBox :"
				+ Integer.parseInt(selectedChapterCheckBox.getId()) + " selectedYearMonthCheckBox :"
				+ Integer.parseInt(selectedYearMonthCheckBox.getId()));
		...
		QuestionLookIpForm upc = new QuestionLookIpForm();
