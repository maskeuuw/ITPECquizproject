...
public class IpChController implements Initializable {
		...
		for (int i = 0; i < chapterCheckId.size(); i++) { // Start at 0 and loop up to size - 1
			for (int j = 0; j < yearmonthCheckId.size(); j++) { // Start at 0 for yearmonthCheckId
				PreparedStatement pst = con
						.prepareStatement("SELECT * FROM ipquestion WHERE c_id = ? AND ym_id = ? AND exam_id = 1");
	...
	public ScrollPane showQuestionList() throws SQLException {
		...
		// Create the header
		Label headerLabel = new Label("Practice IP Questions");
...
//				if (studentID != 0) {
			...
			// Create the result controller with the needed data
			IPResultController resultController = new IPResultController();
	...
	public void setUserAnswerSelections(Map<Integer, String> userAnswerSelections) {
		IpChController.userAnswerSelections = userAnswerSelections;
	}
}
