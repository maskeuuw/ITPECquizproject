...
import application.student.form.ForgetForm;
...
public class ForgotPasswordController {
	...
	void btnNext(ActionEvent event) throws IOException {
		...
		try (Connection con = db.getConnetion()) {
			if (check()) {
				...
				String query1 = "SELECT password FROM student WHERE student_id = ?";
	...
	public boolean check() throws SQLException {
		...
		DatabaseConnection db = new DatabaseConnection();
		try (Connection con = db.getConnetion()) {
			String query1 = "SELECT email FROM student WHERE student_id = ?";
			String query2 = "SELECT student_id FROM student WHERE email = ?";
			...
			try (PreparedStatement pst1 = con.prepareStatement(query2)) {
				...
				if (rs1.next()) {
					getID = rs1.getString("student_id");
				}
			}
