...
public class IPResultController {

	private int totalQuestions = IpChController.limitedQuestions.size();
	private List<Question> reviewQuestions;
	private int[] reviewAnswers;
	private int reviewTotalQuestions;

	// Method to receive data from IpChController
	...
	public BorderPane getResultView() {
		...
		int incorrectOrUnanswered = totalQuestions - IpChController.examTotalMark;
		PieChart.Data correctData = new PieChart.Data("Correct Answers", IpChController.examTotalMark);
		...
		double percentage = ((double) IpChController.examTotalMark / totalQuestions) * 100;
		...
		if (IpChController.examTotalMark >= totalQuestions * 0.6) {
	...
	private String getUserAnswer(Question question, int answerStatus, int questionIndex) {
		// Get the stored user answers from IpChController
		Map<Integer, String> userSelections = IpChController.userAnswerSelections;
	...
	void homeLinkAction(ActionEvent event) throws IOException {
		IpChController.examTotalMark = 0;
