...
import application.admin.form.ForgetForm;
...
public class ForgotPasswordController {
	...
	void btnNext(ActionEvent event) throws IOException {
		...
		try (Connection con = db.getConnetion()) {
			if (check()) {
				...
				String query1 = "SELECT password FROM admin WHERE admin_id = ?";
	...
	public boolean check() throws SQLException {
		...
		DatabaseConnection db = new DatabaseConnection();
		try (Connection con = db.getConnetion()) {
			String query1 = "SELECT email FROM admin WHERE admin_id = ?";
			String query2 = "SELECT admin_id FROM admin WHERE email = ?";
